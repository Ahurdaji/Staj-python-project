...
class User(db.Model, UserMixin):
    ...
    comments = db.relationship('Comment', backref='commenter', passive_deletes=True, overlaps="author_comments")
...
class Comment(db.Model):
    ...
    author = db.relationship('User', backref=db.backref('author_comments', lazy=True), overlaps="commenter,comments")
...
def show_post(post_id):
    ...
    if fetched_post:
        ...
        for comment in fetched_post.post_comments:
            print(f"Comment by {comment.author.username}: {comment.body}".encode('utf-8', errors='replace').decode('utf-8'))
